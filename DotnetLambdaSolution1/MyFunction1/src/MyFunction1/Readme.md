...
WORKDIR /src
COPY ["MyFunction1.csproj", "MyFunction1/"]
RUN dotnet restore "MyFunction1/MyFunction1.csproj"

WORKDIR "/src/MyFunction1"
COPY . .
RUN dotnet build "MyFunction1.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "MyFunction1.csproj" \
...
Execute unit tests
```
cd "MyFunction1/test/MyFunction1.Tests"
dotnet test
```
